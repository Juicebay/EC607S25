...
For a 5% test 
- the .b.orange[classical] SEs reject the .b.orange[true value (zero)] in `r null_df |> filter(se_type == 'classical') |> summarize(mean(p.value < 0.05)) |> as.numeric() |> scales::percent(accuracy = 0.01)` of samples;
- the .b.purple[het.-robust] SEs reject the .b.purple[true value (zero)] in `r null_df |> filter(se_type == 'HC2') |> summarize(mean(p.value < 0.05)) |> as.numeric() |> scales::percent(accuracy = 0.01)` of samples.
